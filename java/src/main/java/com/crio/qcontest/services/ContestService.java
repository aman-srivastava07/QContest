...
public class ContestService{
    ...
    public List<Contestant> runContest(Long contestId, String createdBy) {
        ...
        List<Contestant> contestantList = contestantRepository.findAllByContestId(contestId);  //doubt-01
    ...
    {
        Contest contest = contestRepository.findById(contestId).orElseThrow(()-> new RuntimeException("History for Contest Id: " + contestId + " not found!"));

        //two checks that we missed
        ...
        //Provide a list of contestants in this contest
        List<Contestant> contestantList = contestantRepository.findAllByContestId(contestId);
        //access all elemts of list
        // contestantList.forEach((contestant) ->
        // {});

        //sort in descending order of CurrentContestPoints
        //return contestantList.stream().sorted();
        return contestantList.stream().sorted((c1,c2)-> c2.getCurrentContestPoints() - c1.getCurrentContestPoints()).collect(Collectors.toList());
    }  
}
